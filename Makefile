...
# 14 files were already unformatted before this target existed; failing on
# those would make the gate useless on its first run, and reformatting them
# here would bury unrelated churn in every diff. `make fmt-all` cleans them up
# as a deliberate, separate change.
BASE ?= origin/develop
