...
`cicheck` and `sarif` will then wait for triage and fall back to plain SAST results if it
doesn't complete in time. The wait is bounded by `--timeout` plus `--knoxiq-timeout`, measured
from the start of the run, so time the static scan does not use carries over to KnoxIQ — with
both at their defaults that is 60 minutes in total:
...
- `--include-needs-review` — by default, vulnerabilities KnoxIQ flags as "needs review" are
  excluded from the build decision; this flag includes them. It is a `cicheck` flag; for
  `sarif` set it through `appknox config` or `APPKNOX_INCLUDE_NEEDS_REVIEW` instead.
...
appknox sarif 12345 --output report.sarif
```

Use `--no-knoxiq` to skip the KnoxIQ wait entirely and produce the plain SAST report
immediately. The report is not decorated with AEIS score or exploit likelihood, and keeps every
finding, including the ones flagged as needs review:

```bash
appknox sarif 12345 --output report.sarif --no-knoxiq
```
